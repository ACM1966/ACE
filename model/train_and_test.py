...
def get_dataloaders():
    ...
    train_loader = build_mask_loader(train_embeddings, train_ndv, train_n, train_profile, train_tab_cnt, max_column_len,
                                     shuffle=True)
...
def evaluate(model: nn.Module, loader: DataLoader, device, dataset_name: str):
    ...
    print(dataset_name, ':')
    print(
        f'mean: {np.mean(predicted_q_error):.2f}, 50%: {np.percentile(predicted_q_error, 50):.2f} 75%: {np.percentile(predicted_q_error, 75):.2f} 90%: {np.percentile(predicted_q_error, 90):.2f} 95%: {np.percentile(predicted_q_error, 95):.2f} 99%: {np.percentile(predicted_q_error, 99):.2f}')
...
class Model(nn.Module):
    ...
    def run(self, emb, N, profile, mask):
        ...
        emb_transformed = x
        if args.sample:
            x = torch.concat((emb + emb_transformed, torch.log(N), profile[:, :, 1:]),
                             dim=-1)  # [batch, seq_len, estimate_input_len]
        else:
